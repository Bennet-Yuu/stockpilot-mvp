import { z } from "zod";
import type { SecMetricName, SecPeriodKind, SecSourceMode, SecSnapshotStatus } from "./types";
...
export const secNormalizedFactSchema = secFactProvenanceSchema.extend({ metric: secMetricNameSchema, value: z.number(), derivedFrom: z.array(secFactProvenanceSchema).optional() });
...
export const secRecentFilingSchema = z.object({ accessionNumber: z.string(), form: z.enum(["10-K", "10-Q", "8-K"]), filingDate: z.string(), reportDate: z.string().optional(), primaryDocument: z.string(), sourceUrl: z.string().url(), isXbrl: z.boolean().optional() });
